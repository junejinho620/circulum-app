...
async function seed() {
    ...
    const autoGenerated = await communityRepo.find({
        where: [
            { type: community_entity_1.CommunityType.MAJOR, universityId: uniId },
            { type: community_entity_1.CommunityType.COURSE, universityId: uniId },
        ],
    });
    ...
    let commCreated = 0;
    const communities = [
        { name: 'UofT Campus', slug: 'uoft-campus', description: 'The official anonymous community for all University of Toronto students', type: community_entity_1.CommunityType.CAMPUS },
        { name: 'Classes', slug: 'uoft-classes', description: 'Course discussions, notes, exam help, and study tips', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Confessions', slug: 'uoft-confessions', description: 'Anonymous campus confessions — say what you really think', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Housing', slug: 'uoft-housing', description: 'Roommates, sublets, dorm life, and housing advice', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Events', slug: 'uoft-events', description: 'Campus events, meetups, parties, and social gatherings', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Marketplace', slug: 'uoft-marketplace', description: 'Buy, sell, and trade on campus', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Social', slug: 'uoft-social', description: 'Meet people, hangouts, clubs, and campus life', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Mental Health', slug: 'uoft-mental-health', description: 'Support, resources, well-being, and self-care', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Career & Co-op', slug: 'uoft-career', description: 'Internships, co-op, job hunting, and career advice', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Memes', slug: 'uoft-memes', description: 'Campus memes, humor, and relatable UofT moments', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Photography Club', slug: 'uoft-photography', description: 'Campus through the lens — share your shots', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Board Games', slug: 'uoft-boardgames', description: 'Weekly game nights, meetups, and strategy talk', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'St. George Local', slug: 'uoft-stgeorge', description: 'Spots, deals, and hidden gems near campus', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Kensington Market', slug: 'uoft-kensington', description: 'Food, thrift, and community vibes', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Fitness & Gym', slug: 'uoft-fitness', description: 'Workouts, gym buddies, sports, and health tips', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Food & Cooking', slug: 'uoft-food', description: 'Best eats, recipes, and dining on a student budget', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Music', slug: 'uoft-music', description: 'Playlists, concerts, jam sessions, and music discovery', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Gaming', slug: 'uoft-gaming', description: 'PC, console, mobile gaming — find your squad', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'International Students', slug: 'uoft-international', description: 'Resources, experiences, and community for international students', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Commuters', slug: 'uoft-commuters', description: 'TTC tips, commute rants, and carpool connections', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Lost & Found', slug: 'uoft-lostandfound', description: 'Lost something on campus? Found something? Post here', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Relationships', slug: 'uoft-relationships', description: 'Dating, friendships, and relationship advice', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Startups & Side Projects', slug: 'uoft-startups', description: 'Founders, builders, and side project hustlers', type: community_entity_1.CommunityType.CUSTOM },
        { name: 'Grad School', slug: 'uoft-gradschool', description: 'Applications, research life, and grad student community', type: community_entity_1.CommunityType.CUSTOM },
    ];
    ...
    const postRepo = data_source_1.AppDataSource.getRepository('Post');
    const commentRepo = data_source_1.AppDataSource.getRepository('Comment');
    const pollRepo = data_source_1.AppDataSource.getRepository('Poll');
    const pollOptionRepo = data_source_1.AppDataSource.getRepository('PollOption');
    const studyProfileRepo = data_source_1.AppDataSource.getRepository('StudyBuddyProfile');
    const studySessionRepo = data_source_1.AppDataSource.getRepository('StudySessionParticipant');
    const sessionRepo = data_source_1.AppDataSource.getRepository('StudySession');
    const demoHandles = [
    ...
    const getComm = async (slug) => communityRepo.findOne({ where: { slug, universityId: uniId } });
    const memberRepo2 = data_source_1.AppDataSource.getRepository('CommunityMember');
