...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(auth)` | `/(auth)/login` | `/(auth)/register` | `/(auth)/welcome` | `/(tabs)` | `/(tabs)/communities` | `/(tabs)/create` | `/(tabs)/feed` | `/(tabs)/inbox` | `/(tabs)/profile` | `/_sitemap` | `/campus-map` | `/communities` | `/courses` | `/create` | `/feed` | `/inbox` | `/login` | `/professors` | `/profile` | `/register` | `/study-buddy` | `/timetable` | `/welcome`;
