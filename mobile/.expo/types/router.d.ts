...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(auth)` | `/(auth)/login` | `/(auth)/register` | `/(auth)/welcome` | `/(tabs)` | `/(tabs)/communities` | `/(tabs)/create` | `/(tabs)/feed` | `/(tabs)/inbox` | `/(tabs)/profile` | `/_sitemap` | `/communities` | `/create` | `/feed` | `/inbox` | `/login` | `/profile` | `/register` | `/welcome`;
